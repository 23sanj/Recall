function [A,logP] = baum_welch_cont(X,B_list)
...
logPold = -Inf;
%Estmate:
[A,logP] = reestimate_A(A,B_list); %Here logP  is the log-likelihood

while it< maxIters && abs(logPold - logP) >= err
    %if gt(logPold,logP) == 0
        logPold = logP;
        [A,logP] = reestimate_A(A,B_list); %Re-estimate the model
        it = it + 1;
    %else 
     %   break;
    %end
