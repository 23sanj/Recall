function [P_SEQ,X] = compute_emission_prob(M,R,n_backs)
...
%time: t-- per session
X= linspace(1,4,4); %Fixing the range of alpha-between 1-4: nbacks are 1-4
n_X = size(X,2);
n_sessions = numel(n_backs); %The number of sessions for a subject
P_SEQ = zeros(n_sessions,n_X);
for i=1:n_sessions
    %Applying item response theory:
    alpha = 0.5; %The discrimination parameter is set for now
    n_back = n_backs(i); 
    r = R(i);
    m = M(i);
    %IRT model:
    q_plug = alpha* (X- n_back);
