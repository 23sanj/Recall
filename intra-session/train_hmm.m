...
for k = 1:nSubs
    ...
    if(subject_name ~= '.') %Check to ensure that hidden files are not open
        ...
        if(isempty(files) == 0)
            subList{k} =files;
            [M,R,n_backs] = set_up2(subList{k});  %Getting the required values from all trials in all sessions in a training sequence
...
[A,Pi,loglik]=baum_welch(X,B_list); %Applying Baum Welch to re-estimate the parameters
save('A.mat','A');
save('Pi.mat','Pi');
