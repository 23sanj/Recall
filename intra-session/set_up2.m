...
function [M R n_backs] = set_up(subList)
    ...
    % total_n_back = [total_n_back;N_BACK(foo)];
    % total_TP =[total_TP;TP(foo)];
    % total_FP =[total_FP;FP(foo)];
    % total_TN =[total_TN;TN(foo)];
    % total_FN= [total_FN;FN(foo)];
    % total_TOTAL_STIMULUS =[total_TOTAL_STIMULUS;TOTAL_STIMULUS(foo)];
