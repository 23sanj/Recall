function [P_SEQ,X] = compute_emission_prob(M,R,n_backs,rho,c,d)
...
%time: t-- per session
X= linspace(1,48,96);
 %Fixing the range of alpha-between 1-4: nbacks are 1-4
n_X = size(X,2);
...
for i=1:n_sessions
    ...
    %c = 0.2105;
    q_plug = rho*(X- n_back);
    q= c + (d - c)./ ( 1.0 + exp(-q_plug)); % a vector with 20 values corresponding to the probability 
