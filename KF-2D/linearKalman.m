...
function [] = linearKalman(data_list,n_backs_list)
if nargin == 0 
        ...
        if(subject_name ~= '.') %Check to ensure that hidden files are not open
            ...
            if f ~=1 %All session files must be present in order
                if(isempty(files) == 0)
                    ...
                    t= t+1;
                    [M1,R1,n_backs,data] = set_up3(subList{k});  %Getting the required values from all sessions
                    if isempty(M1) == 1 || isempty(data) == 1
                        continue;
                    end
...
for i=1:5 %Random Restarts to avoid getting stuck in local maxima
    [A, C, B, D, Q, R, x0, V0, loglik] = learn_kalman(data_list,n_backs_list);
    A_rr{i} = A;
    B_rr{i} = B;
    C_rr{i} = C;
    D_rr{i} = D;
